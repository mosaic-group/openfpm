...
void init(sgrid_type & grid, Box<3,real_number> & domain)
{
	...
	Point<3,real_number> p1;
	Point<3,real_number> p2;
	Point<3,real_number> p3;
	...
	Sphere<3,real_number> sph1(p1,0.3);
	Sphere<3,real_number> sph2(p2,0.3);
	Sphere<3,real_number> sph3(p3,0.3);

	Point<3,real_number> u({1.0,1.0,1.0});
	...
	grid.addPoints([spacing_x,spacing_y,spacing_z,u,sph1,sph2,sph3,channel_box] __device__ (int i, int j, int k)
			        {
		                ...
		                real_number distance = vp.norm() / sqrt(3.0f);
