...
template<typename dist_vector_type_ker>
struct ObserverFunctor {
    ...
    //Constructor
    ObserverFunctor(dist_vector_type_ker &vectorDistKer)
    : vectorDistKer(vectorDistKer)
    {
...
void run_stepper_const(dist_vector_type &Particles, std::vector<double> &runtime_v, laplacian_type &Lap, verletList_type& verletList) {
    ...
    ObserverFunctor ObserveAndUpdate(vectorDistKer);
    auto C = getV<0>(Particles);
